...
# qwen/qwen3.6-27b (the model this was originally built against) was
# deprecated by Groq and removed from their model catalog -- confirmed by a
# live 404 model_not_found error in production, then re-verified against
# Groq's current /docs/models and /docs/vision pages (Sept 2026): it no
# longer appears in the production/preview model tables at all. Replaced
# with qwen/qwen3.8-27b, its direct successor (same vision+JSON-mode+
# reasoning-effort capabilities), currently listed as a Preview model. Its
# real limit is 3 images/request (down from qwen3.6-27b's 5) and a 20MB
# per-request image size cap -- MAX_DOCUMENT_PAGES/MAX_DOCUMENT_IMAGE_MB
# below mirror those real API limits so we reject an oversized upload
# before ever calling Groq, not after paying for a guaranteed-to-fail
# request. Being a Preview model, Groq may deprecate this one too without
# much notice -- if this starts 404ing again, re-check
# https://console.groq.com/docs/vision for the current model ID.
GROQ_VISION_MODEL = os.getenv("GROQ_VISION_MODEL", "qwen/qwen3.8-27b")
MAX_DOCUMENT_PAGES = int(os.getenv("MAX_DOCUMENT_PAGES", 3))
